...
Every target must already have a usable own product photo, so appending an image does not replace the website's automatic first-photo fallback. The importer never reorders or deletes photos. It appends the new original with `POST /api/products/{id}/photos`, downloads those stored bytes to verify the SHA, and uses `PUT /api/products/{id}/photos/{photoId}/lead` with `{"role":"CATALOGUE","lead":true}`. It does not set a WEBSITE role, publish family images, or change manual family overview/detail selections. Own product photos use their existing product translations; this path does not create family alt-text records.
