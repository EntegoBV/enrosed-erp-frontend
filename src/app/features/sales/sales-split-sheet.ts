...
import { salesSplitBlockReason, salesSplitPreviewMatches, salesSplitRequest, type SalesSplitQuantities, type SalesSplitOverrides } from './sales-split-state';
      ...
      <div body class="split">
        ...
        @if (eligibility(); as data) {
          ...
          @else if (preview(); as review) {
            ...
            <div class="split__review">
              @for (lane of lanes; track lane.key) {
                ...
                <section class="split__amounts" [class.split__amounts--later]="lane.key === 'later'">
                  ...
                  <h4>{{ lane.title }}</h4><p>{{ part.quantity | num }} stuks @if (lane.key === 'later' && deliveryWeek()) { <span>· {{ deliveryWeek() | weekNl }}</span> }</p>
            ...
            <p class="split__hint">De aantallen blijven samen {{ originalQuantity() | num }} stuks. Je controleert de bedragen in de volgende stap.</p>
          }
        }
      </div>
...
export class SalesSplitSheet implements OnDestroy {
  ...
  async load(): Promise<void> {
    ...
    this.eligibility.set(null); this.quantities.set({}); this.quantityErrors.set({}); this.overrides.set({}); this.deliveryWeek.set(''); this.preview.set(null);
  ...
  laterQuantity(line: SalesSplitLine): number { const value = this.quantities()[line.lineId]; return typeof value === 'number' && Number.isFinite(value) ? value : 0; }
  remaining(line: SalesSplitLine): number { return Math.max(0, line.quantity - this.laterQuantity(line)); }
  ...
  setQuantity(line: SalesSplitLine, value: number | null): void {
    if (this.locked() || !this.eligibility()?.lines.some(item => item.lineId === line.lineId)) return;
  ...
  async check(): Promise<void> {
    ...
    try { request = salesSplitRequest(eligibility, this.quantities(), this.deliveryWeek(), this.overrides(), this.freightRequired()); }
