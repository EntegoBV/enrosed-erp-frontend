      ...
      <app-sheet [title]="draft.direction === 'REFUND' ? (draft.id ? 'Terugbetaling corrigeren' : 'Terugbetaling registreren') : (draft.id ? 'Ontvangst corrigeren' : 'Ontvangst registreren')" (closed)="close()">
        <div body>
          ...
          <div class="form-grid">
            ...
            <label class="field span-2"><span>Bankrekening (dezelfde naam als bij Bank)</span><input class="input" maxlength="120" placeholder="Bijvoorbeeld KBC zichtrekening of IBAN" [ngModel]="draft.bankAccount" (ngModelChange)="patch({ bankAccount: $event })" /></label>
        ...
        <div foot style="display:contents">@if (draft.id) { <button class="btn btn--danger" type="button" [disabled]="busy()" (click)="remove(draft.id)">Verwijderen</button> }<span class="spacer"></span><button class="btn" type="button" [disabled]="busy()" (click)="close()">Annuleren</button><button class="btn btn--primary" type="button" [disabled]="busy()" (click)="save()">{{ busy() ? 'Bewaren…' : 'Bankbeweging bewaren' }}</button></div>
      </app-sheet>
    }
  `,
  styles: `
...
export class SalesReceipts {
  ...
  add(amount?: number): void {
    ...
    this.error.set('');
    this.draft.set({ amount: amount ?? this.summary()?.remainingEur ?? 0, ...receiptLocalParts(Date.now(), timeZone), timeZone, reference: '', bankAccount: '', direction: 'RECEIPT' });
  }
  refund(): void {
    if (!this.canRefund() || this.dirty() || this.busy()) return;
    const timeZone = 'Europe/Brussels';
    this.error.set('');
    this.draft.set({ amount: this.summary()?.refundableEur ?? 0, ...receiptLocalParts(Date.now(), timeZone), timeZone, reference: '', bankAccount: '', direction: 'REFUND' });
  }
  edit(payment: SalesPayment): void {
    if (this.dirty() || this.busy()) return;
    this.error.set('');
    this.draft.set({ id: payment.id, amount: Math.abs(payment.amountEur), direction: payment.amountEur < 0 ? 'REFUND' : 'RECEIPT', bankAccount: payment.bankAccount ?? '', ...receiptLocalParts(payment.receivedAt, payment.timeZone), timeZone: payment.timeZone, reference: payment.reference ?? '' });
  }
  ...
  async issue(): Promise<void> {
    ...
    try {
      ...
      const timeZone = 'Europe/Brussels';
      if ((fresh.paymentSummary?.invoiceTotalEur ?? 0) > 0) this.draft.set({ amount: fresh.paymentSummary?.remainingEur ?? 0, ...receiptLocalParts(Date.now(), timeZone), timeZone, reference: '', bankAccount: '', direction: 'RECEIPT' });
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      this.changed.emit(updated); this.draft.set(null); this.ui.toast('Bankbeweging bewaard en gekoppeld', 'ok');
