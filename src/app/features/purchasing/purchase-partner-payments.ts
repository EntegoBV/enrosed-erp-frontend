    ...
    @if (order().partnerCustomerId != null) {
      <section class="partner-money" aria-label="Partnerfinanciering en ontvangsten">
        ...
        @if (summary(); as summary) {
          ...
          <details class="partner-money__costs"><summary>Kost, financieringsafspraak &amp; resultaat</summary><dl><div><dt>{{ summary.costFinalized ? 'Externe containerkost' : 'Verwachte externe containerkost' }}</dt><dd>{{ summary.forecastExternalEur | eur }}</dd></div><div><dt>Afgesproken financiering</dt><dd>{{ summary.committedAdvanceEur | eur }}</dd></div><div><dt>Uitgereikte voorschotfacturen</dt><dd>{{ summary.invoicedAdvanceEur | eur }}</dd></div><div><dt>Afrekeningen na voorschotten</dt><dd>{{ summary.settlementEur | eur }}</dd></div><div><dt>Gerealiseerd resultaat</dt><dd>{{ summary.recognizedProfitEur | eur }}</dd></div></dl><p class="partner-money__hint">Deze kost- en resultaatbedragen zijn excl. btw. Een voorschot is financiering; het verkoopresultaat ontstaat bij elke uitgegeven veilingafrekening.</p></details>
          <div class="partner-money__docs">
            @for (doc of summary.documents; track doc.id) {
              <article><div><a [routerLink]="['/sales', doc.id]">{{ doc.number }} · {{ doc.purpose === 'PARTNER_SETTLEMENT' ? 'veilingafrekening' : doc.docType === 'FACTUUR' ? 'voorschotfactuur' : 'voorschotofferte' }}</a><small>{{ doc.invoiceTotalEur | eur }} incl. btw @if (doc.docType === 'FACTUUR') { · {{ doc.receivedEur | eur }} ontvangen · {{ doc.remainingEur | eur }} open }</small>@if (doc.creditEur > 0) { <small>{{ doc.creditEur | eur }} credit</small> }</div>@if (doc.docType === 'FACTUUR') { <button class="btn btn--sm" type="button" [disabled]="opening()" (click)="open(doc.id)">Betalingen</button> }</article>
            } @empty { <p class="partner-money__hint">Stel hierboven de factuurtermijnen in en maak per termijn een voorschotfactuur.</p> }
          </div>
          @if (selected(); as selectedDoc) { <div class="partner-money__selected" #selectedInvoice tabindex="-1"><header><b>{{ selectedDoc.order.number }}</b><button class="btn btn--sm" type="button" (click)="selected.set(null)">Sluiten</button></header><app-sales-receipts [view]="selectedDoc" (changed)="received($event)" /></div> }
