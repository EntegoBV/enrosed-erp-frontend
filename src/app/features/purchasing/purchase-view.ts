  ...
  template: `
    @if (view(); as data) {
      ...
      <div class="content purchase-view-page anim-rise erp-workspace erp-workspace--purchase erp-workspace--view">
        ...
        <nav class="purchase-section-nav workflow-nav workflow-nav--wide erp-workspace__nav" aria-label="Onderdelen van deze inkooporder">
          <button class="erp-workspace__nav-item workflow-nav__item" type="button"
                  ...
                  [class.workflow-nav__active]="workspaceSection() === 'purchase-overview'"
            ...
            <span class="workflow-nav__copy"><b>Order</b><small>{{ statusLabel(data.order.status) }}</small></span>
          </button>
          <button class="erp-workspace__nav-item workflow-nav__item" type="button"
          ...
          <button class="erp-workspace__nav-item erp-workspace__nav-item--action workflow-nav__item" type="button"
            ...
            <span class="workflow-nav__copy"><b>Volgende stap</b><small>{{ actionTitle(data.order.status, data.costing.lines.length) }}</small></span>
          </button>
        </nav>
              ...
              <section class="card payments-card erp-workspace__section erp-workspace__support-card"
                ...
                <ul class="report-list">
                  @for (report of data.receiptReports; track $index) {
                    <li [class.report-list__later]="report.source === 'LATER'">
                      <span class="report-list__tag">{{ report.source === 'LATER' ? 'Na uitpakken' : 'Bij ontvangst' }}</span>
                      <span class="report-list__what"><b>{{ report.productName }}</b><small>{{ report.sku || '' }}{{ report.on ? ' · ' + (report.on | dateNl) : '' }}{{ report.actor ? ' · ' + report.actor : '' }}</small></span>
                      <span class="report-list__count">@if (report.damaged) { <b>{{ report.damaged | num }}</b> beschadigd }@if (report.damaged && report.missing) { · }@if (report.missing) { <b>{{ report.missing | num }}</b> te weinig }</span>
                      @if (report.note) { <em class="report-list__note">{{ report.note }}</em> }
                    </li>
                  }
                </ul>
                <p class="report-list__hint">Staat als waarschuwing op de volgende leveranciersorder van deze producten.</p>
              </section>
            }
