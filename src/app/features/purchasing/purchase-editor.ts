...
@Component({
  ...
  imports: [PurchaseSalesLinks, Skeleton, PurchaseQuoteSheet, PurchaseExtraSplit, PurchasePartnerPanel, PurchasePartnerPayments, PurchaseReconciliation, PurchasePaymentResult, PurchasePaymentScope, PurchasePartnerSheet, AuctionSettlementSheet, FormsModule, RouterLink, PageHeader, Diary, ProductPicker, DateField, Sheet, AuthImage,
            ...
            <section class="card payments-card erp-workspace__section"
              ...
              <div class="action-card__head">
                ...
                <h2 id="purchase-payments-title">
                  @if (paymentStateLoading()) { Betalingen laden… } @else if (paymentStateError()) { Betalingen niet actueel } @else if (paidAll() > 0) { {{ paidAll() | eur }} betaald } @else { Nog niets betaald }
                </h2>
                    ...
                    <ol class="instalments">
                      @for (step of plan; track step.label) {
                        <li [class.instalments__item--paid]="step.state === 'paid'" [class.instalments__item--due]="step.state === 'due'">
                          ...
                          @if (step.state === 'due') {
                            <button class="btn btn--sm" type="button" (click)="openPayment(step.amount, step.label, 'SUPPLIER', step.due)">Noteren</button>
                          }
                        </li>
                      }
                    </ol>
      ...
      @if (!desktop.active()) {
        <nav class="purchase-mobile-dock erp-workspace__mobile-dock"
          ...
          @if (phoneStep() < phoneStepLabels.length - 1) {
            <!-- The next step by name: the wide dark button says where it goes,
                 the same dock the sales editor has. -->
            ...
            <span class="purchase-mobile-dock__context purchase-mobile-dock__context--quiet">
              <small>Stap {{ phoneStepLabels.length }} van {{ phoneStepLabels.length }}</small>
  ...
  styles: [`
    ...
    .purchase-mobile-dock__back{width:44px;height:44px;display:grid;flex:none;place-items:center;padding:0;border:0;border-radius:14px;background:var(--surface-2);color:var(--ink);font:inherit;font-size:23px;cursor:pointer}
    .purchase-mobile-dock__back:active{transform:scale(.96)}
    .purchase-mobile-dock__next{display:flex;flex:1;min-width:0;min-height:44px;align-items:center;justify-content:space-between;gap:8px;padding:0 6px 0 14px;border:0;border-radius:14px;background:var(--ink);color:#fff;font:inherit;text-align:left;cursor:pointer;transition:transform .15s ease}
    .purchase-mobile-dock__next:active{transform:scale(.98)}
    .purchase-mobile-dock__next i{width:32px;height:32px;display:grid;flex:none;place-items:center;border-radius:10px;background:rgb(255 255 255/.16);font-size:20px;font-style:normal;line-height:1}
    .purchase-mobile-dock__context{min-width:0;display:grid;flex:1;line-height:1.15}
    .purchase-mobile-dock__context small{overflow:hidden;color:var(--muted);font-size:8.5px;font-weight:650;letter-spacing:.04em;text-overflow:ellipsis;text-transform:uppercase;white-space:nowrap}
    .purchase-mobile-dock__context strong{overflow:hidden;font-size:12.5px;text-overflow:ellipsis;white-space:nowrap}
    .purchase-mobile-dock__next .purchase-mobile-dock__context small{color:rgb(255 255 255/.62)}
    .purchase-mobile-dock__context--quiet{padding-left:4px}
    .purchase-mobile-dock .btn{min-height:44px;margin:0;padding-inline:13px;border-radius:14px}
    .purchase-mobile-dock__pdf,.purchase-mobile-dock__save{flex:none}
    .purchase-mobile-dock__save:disabled{opacity:.58}
    .purchase-mobile-dock__primary{flex:1;min-width:0}
    @media(max-width:360px){.purchase-mobile-dock__pdf{display:none}}
...
export class PurchaseEditor {
  ...
  readonly paying = signal<{ id?: number | null; amount: number; currency: Currency; paidOn: string; label: string; payee: Payee; files: File[]; settles: boolean; instalmentDue: PurchasePayment['instalmentDue'] } | null>(null);
  ...
  editPayment(payment: PurchasePayment): void {
    if (this.payingBusy() || this.paymentStateLoading()) return;
  ...
  readonly payChips = computed(() => {
    ...
    const chips = (planned.length ? planned : [
      { label: '1/3', amount: Math.round((goods / 3) * 100) / 100, due: null },
      { label: '1/2', amount: Math.round((goods / 2) * 100) / 100, due: null },
      { label: '2/3', amount: Math.round((goods * 2 / 3) * 100) / 100, due: null },
  ...
  openPayment(amount?: number, label?: string, payee: Payee = 'SUPPLIER', instalmentDue: PurchasePayment['instalmentDue'] = null): void {
    if (this.payingBusy() || this.paymentStateLoading() || this.paymentStateError()) return;
  ...
  readonly paymentInstalmentOptions = computed(() => {
    ...
    return data && Array.isArray(data.reconciliation?.supplierInstalments)
      ? instalmentsOf(data.order, PAYMENT_TERMS).map(({ due, label }) => ({ due, label })) : [];
  });

  async confirmPayment(): Promise<void> {
    const data = this.view();
    const pay = this.paying();
    if (!data || !pay || this.payingBusy() || this.paymentStateLoading() || this.paymentStateError()) return;
