...
@Component({
  ...
  template: `@if (documents().length) {
    <section aria-label="Gekoppelde verkoopdocumenten"><h3>Verkoopdocumenten van deze container</h3>
      @for (doc of documents(); track doc.order.id) { <a [routerLink]="['/sales', doc.order.id]"><b>{{ doc.order.number }} · {{ kind(doc.order, doc.settlement?.finalSettlement) }}</b><small>{{ partner(doc.order) ? 'Partnerfinanciering / veilingafrekening' : 'Reguliere verkoop · container als herkomst' }}</small></a> }
    </section>
  }`,
